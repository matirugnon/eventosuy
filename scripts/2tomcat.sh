...
if [ $? -ne 0 ]; then
   ...
   echo "    Asegúrate de que el Servidor Central esté corriendo en $PUBLICADOR1_URL"
   read -n 1 -s -r -p "Presiona cualquier tecla para continuar..."
   exit 1
fi
echo
...
echo "========================================"
# Verificar que el archivo WAR de la aplicación web exista
WAR_PATH="/ens/devel01/tpgr15/tpgr15/servidor-web/target/web.war"
if [ ! -f "$WAR_PATH" ]; then
   echo "❌ Error: No se encuentra el archivo WAR de la aplicación web en: $WAR_PATH"
   exit 1
fi
# Copiar el WAR de la aplicación web al directorio de Tomcat
echo "Copiando WAR de la aplicación web a Tomcat..."
cp "$WAR_PATH" "$TOMCAT_DIR/webapps/"
# Verificar si la copia fue exitosa
if [ $? -ne 0 ]; then
   echo ""
   echo "❌ Error al copiar el WAR de la aplicación web"
   echo "    Verifica que tienes permisos de escritura en $TOMCAT_DIR/webapps/"
   exit 1	
fi
echo "✓ WAR web copiado exitosamente"
echo ""
# **Iniciar Tomcat**
echo "========================================"
...
echo ""
echo "Luego accede a:"
echo "   - http://localhost:8080/web/         (Inicio de la aplicación web)"
echo ""
