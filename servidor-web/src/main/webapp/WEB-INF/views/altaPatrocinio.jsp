	...
	<div>
		...
		<div class="content">
			...
			<main class="alta-patrocinio-container">
				<section class="panel">
					<div class="panel-header">Alta de Patrocinio /
						${edicionSeleccionada}</div>
					...
					<form id="formPatrocinio" class="alta-patrocinio-form"
						...
						<label class="form-group"> <span class="label-text">Tipo
								de registro *</span> <select id="tipoRegistro" name="tipoRegistro"
							required>
								<option value="">Seleccionar tipo de registro…</option>
								<c:forEach var="tipo" items="${tiposRegistro}">
									<option value="${tipo.nombre}"
										<c:if test="${tipoRegistroSeleccionado == tipo.nombre}">selected</c:if>>${tipo.nombre}</option>
								</c:forEach>
						</select>
						</label>
						...
						<div id="infoTipoRegistro" class="info-box hidden">
							<c:forEach var="tipo" items="${tiposRegistro}">
								<div id="desc-${tipo.nombre}" class="desc-tipo hidden"
									style="white-space: pre-wrap;">
									<p>
										<strong>Descripción:</strong> ${tipo.descripcion}
									</p>
									<p>
										<strong>Costo:</strong> ${tipo.costo} UYU
									</p>
									<p>
						...
						<div id="formDetalles" class="hidden">
							<label class="form-group"> <span class="label-text">Institución
									*</span> <select name="institucion" required>
									<option value="">Seleccione una institución…</option>
									<c:forEach var="inst" items="${instituciones}">
										<option value="${inst}"
											<c:if test="${institucionSeleccionada == inst}">selected</c:if>>${inst}</option>
									</c:forEach>
							</select>
							</label> <label class="form-group"> <span class="label-text">Nivel
									de Patrocinio *</span> <select name="nivelPatrocinio" required>
									<option value="">Seleccione nivel…</option>
									<option value="Platino"
										<c:if test="${nivelSeleccionado == 'Platino'}">selected</c:if>>Platino</option>
									<option value="Oro"
										<c:if test="${nivelSeleccionado == 'Oro'}">selected</c:if>>Oro</option>
									<option value="Plata"
										<c:if test="${nivelSeleccionado == 'Plata'}">selected</c:if>>Plata</option>
									<option value="Bronce"
										<c:if test="${nivelSeleccionado == 'Bronce'}">selected</c:if>>Bronce</option>
							</select>
							</label> <label class="form-group"> <span class="label-text">Aporte
									Económico (UYU) *</span> <input name="aporteEconomico" type="number"
								min="0" step="0.01" placeholder="Ej. 10000"
								value="${aporteIngresado}" required />
							</label> <label class="form-group"> <span class="label-text">Cantidad
									de registros gratuitos *</span> <input name="registrosGratuitos"
								type="number" min="0" step="1" placeholder="Ej. 10"
								value="${registrosIngresados}" required />
							</label> <label class="form-group"> <span class="label-text">Código
									de Patrocinio *</span> <input name="codigoPatrocinio" type="text"
								maxlength="20" placeholder="Código único"
								value="${codigoIngresado}" required />
							</label>
						</div>
