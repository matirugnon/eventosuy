			...
			<section class="auth-container">
				<div class="auth-card">
					...
					<form class="auth-form"
						...
						<!-- Campos no editables -->
						<div class="grid-2">
							<label class="form-group"> <span>Nickname</span> <input
								value="${usuario.nickname}" readonly class="readonly" />
							</label> <label class="form-group"> <span>Correo
									electrónico</span> <input value="${usuario.correo}" readonly
								class="readonly" />
							</label>
						</div>
						...
						<div class="grid-2">
							<label class="form-group"> <span>Nombre</span> <input
								name="nombre"
								value="${param.nombre != null ? param.nombre : usuario.nombre}"
								required maxlength="60" />
							</label>

							<!-- Campo específico para Asistente -->
							<c:if test="${tipoUsuario == 'Asistente'}">
								<label class="form-group"> <span>Apellido</span> <input
									name="apellido"
									value="${param.apellido != null ? param.apellido : asistente.apellido}"
									required maxlength="60" />
								</label>
							</c:if>
						...
						<c:choose>
							<c:when test="${tipoUsuario == 'Asistente'}">
								<div class="grid-2">
									<label class="form-group"> <span>Fecha de
											nacimiento</span> <input name="fechaNac"
									...
									</label> <label class="form-group"> <span>Institución</span> <select
										name="institucion">
							...
							<c:when test="${tipoUsuario == 'Organizador'}">
								<label class="form-group"> <span>Descripción</span> <textarea
								...
								<label class="form-group"> <span>Sitio web</span> <input
									name="web" type="url"
						...
						<!-- Imagen -->
						<label class="form-group"> <span>Imagen</span> <input
							name="imagen" type="file" accept="image/*" /> <span class="help">
								<c:choose>
                    ...
                    Sin imagen actual. Selecciona una imagen para agregar.
						...
						<!-- Seguridad -->
						<div class="grid-2">
							<label class="form-group"> <span>Nueva contraseña</span>
								<input type="password" name="password" minlength="6"
								placeholder="Dejar vacío para mantener actual" />
							</label> <label class="form-group"> <span>Confirmar
									contraseña</span> <input type="password" name="confirm" minlength="6"
								placeholder="Confirmar nueva contraseña" />
							</label>
						</div>
						<p class="help">El nickname y el correo no se pueden
							modificar. Si no deseas cambiar la contraseña, deja los campos
							vacíos.</p>

						<div style="display: flex; gap: .5rem; margin-top: .5rem;">
							<button class="btn-primary" type="submit">Guardar
								cambios</button>
							<a class="btn-outline"
								href="${pageContext.request.contextPath}/miPerfil">Cancelar</a>
						</div>
					</form>
				</div>
			</section>
