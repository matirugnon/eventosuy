...
public class AltaEventoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Convertir array de categorías a Set
            Set<String> categoriasSet = new java.util.HashSet<>();
            for (String categoria : categoriasSeleccionadas) {
                categoriasSet.add(categoria);
            }

            // Crear evento (el método original no soporta imagen, se omite por ahora)
            ...
            mostrarFormularioConError(request, response, "Debe seleccionar al menos una categoría");
        } catch (FechaInvalidaException e) {
            mostrarFormularioConError(request, response, "La fecha del evento no es válida");
        } catch (Exception e) {
    ...
    private String validarDatos(String nombre, String sigla, String descripcion, String[] categorias) {
        ...
        if (sigla.length() > 15) {
            return "La sigla no puede tener más de 15 caracteres";
        }
        ...
        if (descripcion.length() > 800) {
            return "La descripción no puede tener más de 800 caracteres";
        }
