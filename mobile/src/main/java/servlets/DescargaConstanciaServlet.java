...
public class DescargaConstanciaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!"asistente".equals(role)) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN, "Solo los asistentes pueden descargar constancias");
            return;
        }
        ...
        try {
            ...
            // Verificar que el usuario logueado sea el mismo asistente
            String usuarioSesion = (String) session.getAttribute("usuario");
            if (!asistente.equals(usuarioSesion)) {
            ...
            if (!registro.isAsistio()) {
                response.sendError(HttpServletResponse.SC_FORBIDDEN, "Debes confirmar tu asistencia antes de descargar la constancia");
                return;
            }
            ...
            // Obtener información del asistente (getDTUsuario retorna DtUsuario que puede ser DtAsistente)
            DtUsuario usuarioInfo = publicadorUsu.getDTUsuario(asistente);
            
            // Verificar que sea un asistente para obtener apellido
            ...
            // Por el momento, no podemos determinar el código de patrocinio específico
            // ya que no está incluido en el DTO SOAP del registro
    ...
    private void generarPDF(HttpServletResponse response, DtRegistro registro, 
        ...
        try {
            ...
            try {
                String bannerPath = "C:\\Users\\Mati\\eclipse-workspace\\tpgr15\\mobile\\src\\main\\webapp\\img\\banner.png";
            ...
            table.addCell(createDataCell(
                edicionInfo.getFechaInicio().getDia() + "/" + 
                edicionInfo.getFechaInicio().getMes() + "/" + 
                edicionInfo.getFechaInicio().getAnio() + 
                " - " + 
                edicionInfo.getFechaFin().getDia() + "/" + 
                edicionInfo.getFechaFin().getMes() + "/" + 
                edicionInfo.getFechaFin().getAnio()
            ));
